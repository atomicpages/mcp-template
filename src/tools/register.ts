...
/**
 * Extracts the `body` sub-schema from a generated `z{Endpoint}Data` schema
 * (which has shape `{ body, path, query }`) and returns it as a flat record
 * suitable for MCP `inputSchema`.
 *
 * AGENT TODO: if your generator emits a different envelope (e.g. flat schemas,
 * or `{ params, body }`), swap `outerShape.body` for whichever field maps to
 * the agent-facing input. For hand-written tools you can pass a flat object
 * schema and this still works (it'll fall through to `{}` then you should
 * pass `inputSchema` differently — see `registerWorkflowTool` for the
 * hand-written input pattern).
 */
function extractInputSchema(dataSchema: z.ZodTypeAny): ZodShape {
 ...
 */
function is__SERVICE_PASCAL__Error(data: unknown): data is __SERVICE_PASCAL__ErrorResponse {
  return (
    typeof data === "object" &&
    data !== null &&
    "success" in data &&
...
function format__SERVICE_PASCAL__Error(data: __SERVICE_PASCAL__ErrorResponse): string {
      ...
      try {
        ...
        if (is__SERVICE_PASCAL__Error(result)) {
          return {
            isError: true,
            content: [
              { type: "text" as const, text: format__SERVICE_PASCAL__Error(result) },
            ],
          };
        }
        ...
        if (is__SERVICE_PASCAL__Error(error)) {
          return {
            isError: true,
            content: [
              { type: "text" as const, text: format__SERVICE_PASCAL__Error(error) },
            ],
          };
        }
