...
const __SERVICE_KEBAB__StrictTenantCredentialsAls = new AsyncLocalStorage<boolean>();
...
function contextHasKeys(context: __SERVICE_PASCAL__HttpRequestContext): boolean {
...
export function install__SERVICE_PASCAL__PerRequestAuthInterceptor(): void {
  ...
  client.interceptors.request.use(async (request, options) => {
    ...
    const strictTenant = __SERVICE_KEBAB__StrictTenantCredentialsAls.getStore() === true;
...
export function enable__SERVICE_PASCAL__HttpHeaderCredentialBridge(): void {
  ...
  set__SERVICE_PASCAL__CredentialResolver(__SERVICE_KEBAB__CredentialResolverFromContext);
}
  ...
  return async (req, opts) => {
    ...
    if (requireTenant) {
      return __SERVICE_KEBAB__StrictTenantCredentialsAls.run(true, () =>
        run__SERVICE_PASCAL__HttpRequestContext(ctx, () => handleRequest(req, opts)),
      );
    }

    return run__SERVICE_PASCAL__HttpRequestContext(ctx, () => handleRequest(req, opts));
  };
}
