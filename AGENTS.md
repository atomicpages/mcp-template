...
TypeScript **MCP server** for the __SERVICE_TITLE__ API: tools wrap the HTTP API
...
> TODO(__SERVICE_TITLE__): once you generate the SDK, add a row for
...
2. In the appropriate `src/tools/<domain>.ts`, add a `register__SERVICE_PASCAL__Tool`
   block: stable `__SERVICE_KEBAB___*` name, clear `title`/`description`,
   correct `dataSchema`, `sdkFn`, and `readOnly: false` only for mutating
   operations.
...
- [ ] Replace `src/client.ts` with a real client (OpenAPI codegen or hand-written).
- [ ] Update `configure__SERVICE_PASCAL__Client` in `src/__SERVICE_KEBAB__-mcp.ts`
      with the correct base URL and auth scheme (Basic / Bearer / custom header).
...
- [ ] Adjust the success-envelope assumption (`{ success, results }`) in
      ...
      __SERVICE_TITLE__ uses a different shape.
