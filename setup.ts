...
function parseArgs(argv: string[]): {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--kebab" || arg === "--pascal" || arg === "--upper" || arg === "--title") {
...
function validateTokens(t: Tokens): void {
  if (!/^[a-z][a-z0-9-]*$/.test(t.kebab)) {
    throw new Error(`--kebab must be lowercase letters/digits/hyphens, got "${t.kebab}"`);
  }
...
async function main(): Promise<void> {
  ...
  for (const file of renamableFiles) {
    ...
    pathChanges++;
    if (dryRun) {
      console.error(`[rename] ${relative(root, file)} -> ${relative(root, target)}`);
    } else {
  ...
  for (const dir of renamableDirs) {
    ...
    pathChanges++;
    if (dryRun) {
      console.error(`[rename] ${relative(root, dir)} -> ${relative(root, target)}`);
    } else {
